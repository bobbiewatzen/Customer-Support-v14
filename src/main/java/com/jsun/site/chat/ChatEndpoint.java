...
public class ChatEndpoint
{
        ...
        try
        {
            if(this.principal == null)
            {
                ...
                session.close(new CloseReason(
                        ...
                        this.messageSource.getMessage("error.chat.not.logged.in", null, this.locale)
                ));
                return;
            }
            ...
            else
            {
                ...
                if(result == null)
                {
                    ...
                    session.close(new CloseReason(
                            ...
                            this.messageSource.getMessage("error.chat.no.session", null, this.locale)
                    ));
                    return;
                }
                this.chatSession = result.getChatSession();
                ...
                this.otherLocale = EndpointConfigurator.getExposedLocale(this.otherWsSession);
                session.getBasicRemote().sendObject(this.cloneAndLocalize(this.chatSession.getCreationMessage(), this.locale));
                session.getBasicRemote().sendObject(this.cloneAndLocalize(result.getJoinMessage(), this.locale));
                this.otherWsSession.getBasicRemote()
                        .sendObject(this.cloneAndLocalize(result.getJoinMessage(), this.otherLocale));
            }
    ...
    public void onError(Throwable e)
    {
        ...
        synchronized(this)
        {
            if(this.closed)
                return;
            this.close(ChatService.ReasonForLeaving.ERROR, "error.chat.closed.exception");
        }
    }
        ...
        if(message != null)
        {
            ...
            synchronized(this.wsSession)
            {
                if(this.wsSession.isOpen())
                {
                    try
                    {
                        this.wsSession.getBasicRemote().sendObject(this.cloneAndLocalize(message, this.locale));
                        this.wsSession.close(new CloseReason(closeCode, this.messageSource.getMessage(reasonCode, null, this.locale)));
                    }
                    catch(IOException | EncodeException e)
                    {
            ...
            if(this.otherWsSession != null)
            {
                ...
                synchronized(this.otherWsSession)
                {
                    if(this.otherWsSession.isOpen())
                    {
                        try
                        {
                            this.otherWsSession.getBasicRemote().sendObject(this.cloneAndLocalize(message, this.otherLocale));
                            this.otherWsSession.close(new CloseReason(closeCode, this.messageSource.getMessage(reasonCode, null, this.otherLocale)));
                        }
                        catch(IOException | EncodeException e)
                        {
    ...
    private ChatMessage cloneAndLocalize(ChatMessage message, Locale locale) {
    	...
    	message.setLocalizedContent(this.messageSource.getMessage(message.getContentCode(), message.getContentArguments(), locale));
    	return message;
    }
    ...
    public static class EndpointConfigurator extends SpringConfigurator
    {
        ...
        private static final String LOCALE_KEY = "com.jsun.ws.user.locale";
